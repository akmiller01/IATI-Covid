...
agg = subset(agg, transaction_type %in% disb & iati_identifier %in% unique(clean_air$iati_identifier))
agg$usd_disbursement = as.numeric(agg$usd_disbursement)
agg_tab = agg[,.(sum_usd_disbursement=sum(usd_disbursement,na.rm=T)),by=.(iati_identifier)]
fwrite(agg_tab,"clean_air_sum.csv")
