...
agg = subset(
  ...
  secondary_reporter %in% c("0","false") &
    ...
    transaction_type %in% c("E", "D", "3", "4") & (
      grepl("TD",recipient_country_codes) |
        grepl("TCD", recipient_country_codes)
    )
)
...
agg = subset(agg.split.long,recipient.country.codes %in% c("TD","TCD"))
...
fwrite(agg,"chad_query_to-06302020.csv")
